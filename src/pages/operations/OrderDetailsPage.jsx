...
const STATUS_META = {
  ...
  ESCALATED: 'border-orange-200 bg-orange-50 text-orange-800 dark:border-orange-500/30 dark:bg-orange-500/10 dark:text-orange-200',
...
export function OrderDetailsPage() {
  ...
  useEffect(() => {
    ...
    apiInstance
      ...
      .catch(() => {
        if (!controller.signal.aborted) setDepots([])
      })
      .finally(() => {
        if (!controller.signal.aborted) setIsLoadingDepots(false)
      })
        ...
        {isLoading ? (
          ...
          <>
            ...
            <SmoothSlideOver
              ...
              title="Edit order"
              description={`Update status, fulfillment depot, payment, and delivery for ${order.order_number || order.order_id}.`}
              footer={
                form ? (
              ...
              {form && (
                <div className="space-y-6 pb-20">
                  ...
                  <div>
                    ...
                    >
                      <option value="">
                        {isLoadingDepots ? 'Loading depots…' : depots.length === 0 ? 'No depots available' : 'Select depot'}
                      </option>
                      {depots.map((depot) => (
                        <option key={depot.id} value={depot.id}>
                          {depot.depot_name || depot.name || depot.id}
                        </option>
                      ))}
                    </select>
                    <p className="mt-1.5 text-xs text-zinc-500 dark:text-zinc-400">
                      Reassign this order to another depot for fulfillment and routing.
                    </p>
                  </div>
                  <div>
