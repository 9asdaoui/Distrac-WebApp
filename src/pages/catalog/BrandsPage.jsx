...
export function BrandsPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setIsLoading(true)
      try {
        const res = await apiInstance.get('/brands', { signal: controller.signal })
      ...
      <AnimatedPage>
        <div className="space-y-6">
          <div className="flex flex-col gap-4 rounded-2xl border border-gray-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-900 md:flex-row md:items-center md:justify-between">
            <div>
              <h1 className="text-2xl font-semibold text-zinc-900 dark:text-zinc-100">Brands</h1>
              <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Manage product brands and their metadata.</p>
            </div>
