...
function ReviewPanel({ proposal, depotsById, industriesById, onClose, onAccept, onReject, isSubmitting }) {
  ...
  return (
    ...
    >
      ...
      <div className="mb-4 rounded-xl border border-zinc-200 bg-zinc-50 p-4 text-sm text-zinc-700 dark:border-zinc-800 dark:bg-cc-surface dark:text-zinc-200">
      ...
      <div className="overflow-hidden rounded-xl border border-gray-200 dark:border-zinc-800">
        <table className="min-w-full text-left text-sm">
          ...
          <tbody>
            {items.map((item) => (
              <tr key={item.id || `${item.product_id}-${item.quantity}`} className="border-t border-gray-100 dark:border-zinc-800">
                <td className="px-4 py-3 text-zinc-800 dark:text-zinc-200">{item.products?.name || item.product?.name || item.product_id}</td>
            ...
            {items.length === 0 && (
              <tr>
                <td className="px-4 py-8 text-center text-zinc-500" colSpan={3}>No items.</td>
              </tr>
            )}
          </tbody>
        </table>
...
/**
 * Proposal inbox table + review slide — embeddable in Stock tab or standalone page.
 */
export function ProposalsInboxPanel({ onPendingCountChange, showIntro = false } = {}) {
  ...
  useEffect(() => {
    onPendingCountChange?.(visibleProposals.length)
  }, [visibleProposals.length, onPendingCountChange])
  ...
  return (
    <>
      {showIntro ? (
        <div className="mb-4">
          <h2 className="text-lg font-semibold text-zinc-900 dark:text-zinc-100">Proposal inbox</h2>
          <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
            ...
            <table className="min-w-full text-left text-sm">
              ...
              <tbody>
                {visibleProposals.map((proposal) => (
                  <tr key={proposal.id} className="border-b border-gray-200 dark:border-zinc-800">
                    <td className="px-6 py-4 font-medium text-zinc-900 dark:text-zinc-100">
                      {depotsById[proposal.depot_id]?.depot_name || proposal.depot_id}
                    </td>
                ...
                {visibleProposals.length === 0 && (
                  <tr>
                    <td className="px-6 py-10 text-center text-zinc-500 dark:text-zinc-400" colSpan={5}>
...
export default ProposalsInboxPanel
