...
export const railInputClass =
  'w-full rounded-lg border border-zinc-700 bg-zinc-900 px-3 py-2 text-sm text-zinc-100 outline-none placeholder:text-zinc-500 focus:border-zinc-500'
...
export function MapRailShell({
  ...
  return (
    <div className="relative flex h-full w-full flex-col">
      ...
      {overlay && (
        <div className="absolute inset-0 z-20 flex flex-col overflow-hidden bg-[#1c1c1e]">
          {overlay}
        </div>
      )}
    </div>
...
export function MapRailCreateOverlay({ title, subtitle, onClose, children, footer }) {
  return (
    <>
      ...
      {footer && <div className="shrink-0 border-t border-zinc-800 px-4 py-3">{footer}</div>}
    </>
  )
}
